			...
			function v(e) {
				...
				if (
					...
					if (!a) {
						...
						if (
							(a || i('scroll', s), t.params.autoplay && t.params.autoplayDisableOnInteraction && t.autoplay.stop(), n === t.minTranslate() || n === t.maxTranslate())
						)
							return !0
					}
				} else {
...
function popupAllClose(id) {
	$('.popup').each(function (k, e) {
		if ('#' + $(e).attr('id') != id) {
			popupClose('#' + $(e).attr('id'))
		}
	})
}
...
function popupOpen(id) {
	...
	$(id).fadeIn(1, function () {
		var popupHeight = parseInt($(id + ' .popup-container').innerHeight() + 100)

		$(id + ' .popup-overlay').css({ opacity: '1', 'min-height': popupHeight + 'px' })

		if (width <= 992 || popupHeight > height) {
			$(id + ' .popup-container').css({ transform: 'translate(-50%,0%)', top: '30px', opacity: '1' })
		} else {
			$(id + ' .popup-container').css({ transform: 'translate(-50%,-50%)', opacity: '1' })
		}
	...
	$(window).resize(function () {
		...
		let ph = $(id + ' .popup-container').innerHeight()

		if (w <= 992 || ph > h) {
			$(document.body)
				.find(id + ' .popup-container')
				.css({ transform: 'translate(-50%,0%)', top: '30px', opacity: '1' })
		} else {
			$(document.body)
				.find(id + ' .popup-container')
...
/*-- Red black wheel --*/
;(function () {
	const wheel = document.querySelector('.whrb-wheel')
	if (!wheel) return

	const nums = [
		{ num: 10, percent: 0.11, klass: '_black' },
		{ num: 5, percent: 0.15, klass: '_red' },
		{ num: 24, percent: 0.1, klass: '_black' },
		{ num: 16, percent: 0.12, klass: '_red' },
		{ num: 33, percent: 0.2, klass: '_black' },
		{ num: 1, percent: 0.15, klass: '_red' },
		{ num: 20, percent: 0.14, klass: '_black' },
		{ num: 14, percent: 0.31, klass: '_red' },
		{ num: 31, percent: 0.15, klass: '_black' },
		{ num: 9, percent: 0.5, klass: '_red' },
		{ num: 22, percent: 0.17, klass: '_black' },
		{ num: 18, percent: 0.5, klass: '_red' },
		{ num: 29, percent: 0.15, klass: '_black' },
		{ num: 7, percent: 0.7, klass: '_red' },
		{ num: 28, percent: 0.3, klass: '_black' },
		{ num: 12, percent: 0.4, klass: '_red' },
		{ num: 35, percent: 0.5, klass: '_black' },
		{ num: 3, percent: 0.5, klass: '_red' },
		{ num: 26, percent: 0.37, klass: '_black' },
		{ num: 0, percent: 0.2, klass: '_green' },
		{ num: 32, percent: 0.47, klass: '_red' },
		{ num: 15, percent: 0.15, klass: '_black' },
		{ num: 19, percent: 0.2, klass: '_red' },
		{ num: 4, percent: 0.2, klass: '_black' },
		{ num: 21, percent: 0.3, klass: '_red' },
		{ num: 2, percent: 0.4, klass: '_black' },
		{ num: 25, percent: 0.5, klass: '_red' },
		{ num: 17, percent: 0.6, klass: '_black' },
		{ num: 34, percent: 0.2, klass: '_red' },
		{ num: 6, percent: 0.15, klass: '_black' },
		{ num: 27, percent: 0.7, klass: '_red' },
		{ num: 13, percent: 0.3, klass: '_black' },
		{ num: 36, percent: 0.4, klass: '_red' },
		{ num: 11, percent: 0.9, klass: '_black' },
		{ num: 30, percent: 0.3, klass: '_red' },
		{ num: 8, percent: 0.8, klass: '_black' },
		{ num: 23, percent: 0.3, klass: '_red' },
	]
	...
	const $numPriceInp = $('.js-num-price'),
		$colorPriceInp = $('.js-colors-price')

	const renderItem = () => {
		nums.forEach((num, index) => {
			const numLi = document.createElement('li')

			numLi.style.transform = `
                rotate(${rotate * index}deg)
                skewY(-${skewY}deg)
            `

			numLi.classList.add(num.klass)

			numLi.innerHTML = `
                <span style="transform: skewY(${skewY}deg) rotate(${rotate / 2}deg)">
                ${num.num}                    
                </span>`

			wheel.appendChild(numLi)
		})
	}
	...
	const getNum = winPercent => {
		let currentPercent = 0
		let list = []

		nums.forEach((num, index) => {
			currentPercent = num.percent
			winPercent <= currentPercent && list.push({ ...num, index })
		})

		//Если нет совпадение
		if (!list.length) {
			return getNum(Math.random())
		}
	...
	const showNum = obj => {
		setTimeout(() => {
			...
			$('.js-whrb-begin').prop('disabled', false)
			$('.wboard').find('[data-wval]').removeClass('disabled')
			$('.wboard').find('._chip').removeClass('_chip')
	...
	const generateWin = obj => {
		let priceWin = 0,
			numPrice = parseFloat($numPriceInp.val()) || 0,
			colorPrice = parseFloat($colorPriceInp.val()) || 0

		if (wvals.includes(obj.klass)) {
			priceWin += 2 * colorPrice
		}

		if (wvals.includes(obj.num)) {
			priceWin += 36 * numPrice
		}

		if (priceWin > 0) {
			$('#popup-whrb-win').find('.popup-slot-win__price').html(`${priceWin} ₽`)
			popupOpen('#popup-whrb-win')
		}
	}

	const start = () => {
		const winPercent = Math.random()
		const num = getNum(winPercent)

		$('.js-whrb-begin').prop('disabled', true)
		$('.wboard').find('[data-wval]').addClass('disabled')

		clearInterval(timerInterval)
		xtimer('.wboard-timer', 1, () => {})

		currentRotate += 360 * 10

		rotateWhell(currentRotate, num.index)
		showNum(num)
	}

	const renderChips = wvals => {
		//colors
		if (wvals.includes('_red') || wvals.includes('_black')) {
			$('.bcounter--colors').show()

			if (wvals.includes('_red') && wvals.includes('_black')) {
				$('.js-color-chips').html('')
			} else {
				$('.js-color-chips').html(`<span class="_c1"></span>`)
			}
		} else {
			$('.bcounter--colors').hide()
			$('.js-color-chips').html(`<span class="_c1"></span><span class="_c2"></span>`)
		}

		//nums
		let x = 0
		wvals.forEach(item => {
			if (item == '_red' || item == '_black') return

			x++
		})

		x == 0 && $('.js-num-chips').html(`<span class="_c1"></span><span class="_c2"></span><span class="_c3"></span>`)
		x == 1 && $('.js-num-chips').html(`<span class="_c1"></span><span class="_c2"></span>`)
		x == 2 && $('.js-num-chips').html(`<span class="_c1"></span>`)
		x == 3 && $('.js-num-chips').html('')
	...
	// selected numbers
	$('.wboard').on('click', '[data-wval]', function () {
		if ($(this).hasClass('disabled')) return

		//selected 3
		if ($('.wboard-nums ._chip').length >= 3 && !$(this).hasClass('_chip') && Number.isInteger($(this).data('wval'))) return

		$(this).toggleClass('_chip')
		...
		$('.wboard')
			.find('._chip')
			.each((index, el) => {
				wvals.push($(el).data('wval'))
			})

		if (!timerBegin) {
			timerBegin = true
			timerInterval = xtimer('.wboard-timer', 90 * 1000, start)
		}
	...
	//bcounters
	$('.bcounter').on('click', function (e) {
		const $el = $(e.target)
		const inp = $(this).find('.inp')
		let min,
			max,
			step,
			inpVal = +inp.val()

		if ($el.closest('.bcounter--nums').length) {
			min = 5
			max = 1000
			step = 5
		} else if ($el.closest('.bcounter--colors').length) {
			min = 30
			max = 500
			step = 10
		}

		if ($el.hasClass('bcounter__down') && inpVal > min) {
			inpVal -= step
		} else if ($el.hasClass('bcounter__up') && inpVal < max) {
			inpVal += step
		}
	...
	$('.js-whrb-begin').on('click', function () {
		if (wvals.length > 0) start()
	})
...
$(function () {
	...
	$('.lang-btn').on('click', function () {
		$(this).closest('.lang').toggleClass('lang--open')
	})

	//sort
	$('.js-cfilters-sort').on('click', () => {
		$('.cfilters-sort').toggleClass('_open')
	})
	...
	//Смена цитаты каждые 30 секунд
	if ($('.whrb-wrinfo').length) {
		let i = 0
		const words = [
			'“Никогда не думай о том, что можешь проиграть, это отпугивает удачу”',
			'“Удача не приходит одна. Всегда приводит с собой коварных друзей”',
			'“Угадать не дано<br/>Никому наперёд,<br/>Может быть, как в кино<br/>Нам с тобой повезёт”',
			'“Капля отваги дороже моря удачи”',
			'“Если кому-нибудь везёт, не завидуй ему, а порадуйся с ним вместе, и его удача будет твоей!”',
			'“В момент отчаяния Фортуна благоволит более наглым”',
		...
		const timerInt = setInterval(function () {
			...
			$('.whrb-wrinfo > p').fadeOut(200, () => {
				$('.whrb-wrinfo > p').html(words[i]).fadeIn(200)
			})
		}, 30 * 1000)
	}
